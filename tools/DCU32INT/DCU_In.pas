...
begin
  Result := Str;
  while (L>0)and(Result^<>#0) do begin
    Inc(Result);
    Dec(L);
  end;
end;
...
function SarLong(Value: LongInt; Bits: Byte): LongInt;
var
  Divisor: LongInt;
begin
  Divisor := 1 shl Bits;
...
begin
  ...
  if B[0] and $1=0 then begin
    Result := SB;
    Result := SarLong(Result,1);
   end
  else begin
    B[1] := ReadByte;
    if B[0] and $2=0 then begin
      Result := W;
      Result := SarLong(Result,2);
     end
    else begin
      B[2] := ReadByte;
      B[3] := 0;
      if B[0] and $4=0 then begin
        RL.i := ShortInt(B[2]);
        Result := L;
        Result := SarLong(Result,3);
       end
      else begin
        B[3] := ReadByte;
        if B[0] and $8=0 then begin
          Result := L;
          Result := SarLong(Result,4);
         end
        else begin
          B[4] := ReadByte;
          Result := R4.L;
