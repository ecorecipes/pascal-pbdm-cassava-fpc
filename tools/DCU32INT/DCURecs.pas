...
TLocalDecl = class(TNameDecl)
  ...
  hDT: TDefNDX;
  NDXB: PtrInt;//B: Byte; //Interface only; may hold a pointer during analysis
...
TEnumDef = class(TRangeBaseDef)
  ...
  CStart: TConstDecl;
  NameTbl: TList;
  HasEq: Boolean; //Some const was defined by �=�prev and not included into NameTbl
  constructor Create;
  destructor Destroy; override;
  function ShowValue(DP: Pointer; DS: Cardinal): integer {Size used}; override;
  procedure Show; override;
...
begin
  ...
  case Sz of
    ...
    { FPC on this target aliases Extended to Double, duplicating the Double case. }
    { FPC aliases Real to Double in this build, which duplicates the Double case. }
  else
    Ok := false;
  end ;
  if Ok then begin
  ...
  while DeclL<>Nil do begin
    ...
    if (Decl is TLocalDecl)and(TLocalDecl(Decl).GetTag = arFld)and Decl.Name^.IsEmpty then begin
      ...
      if PropDecl<>Nil then begin
        ...
        TLocalDecl(Decl).NDXB := PtrInt(GetObjFldByOfs(TLocalDecl(Decl).NDX{Ofs},TSz{QSz},Pointer(FldUnit)));
      end ;
    end ;
    DeclL := DeclL.Next;
  end ;
